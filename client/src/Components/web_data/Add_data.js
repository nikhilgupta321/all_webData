...
const Add_data = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (jwt) {
        const response = await axios.post('http://localhost:8080/api/webdata', {
  ...
  return (
    <>
      <div className="bar-add gap-4">
        <Link to="/webdata" className="w-auto p-2 text-center bg-gray-400 rounded text-white uppercase text-sm">
      ...
      <form className="grid grid-cols-3 gap-4 p-4 mt-14" encType="multipart/form-data">
        ...
        <div className="col-span-2 row-span-4">
          <label htmlFor="content" className="uppercase">Your Content</label>
          <ReactQuill
            id="content"
